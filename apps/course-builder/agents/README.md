...
Key facts: Vite SPA (port 5176), `react-router-dom`, `@supabase/supabase-js` direct from the client (no auth/RLS yet), dnd-kit reordering, RTL Hebrew content inside the shared `PageLayout`. Env: `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` in `.env`.
