...
|---|---|---|
| `banner` | title (שם הבאנר), imageUrl, notes | Wide 4:1 placeholder: default label `באנר - {page numbering} \| {page title}` (overrides to custom `title` when set); image URL fitted with `object-contain` (full image visible, letterboxed in the 4:1 frame) when set; "ללא קובץ" when no URL or the image fails to load. Click opens image URL in a new tab in edit/implement only — preview has no link. Lighter top strip (`#6B6B6B`) in edit only — hidden in implement/preview. Edit settings above placeholder (name placeholder shows default + image URL + הערות); toggle via card header or chevron. New banners open settings. No subtitle. In all modes, title overlay is hidden only when the image actually paints; if a URL is set but the image does not show (broken/blocked URL), the banner name (+ "ללא קובץ") stays visible. Implement: notes under the block when set; preview: notes hidden. |
| `video` | title (optional display name), url, provider ('youtube' \| 'panopto' \| 'other'), notes | Black aspect-video placeholder in edit/implement: default label `{page title} \| סרטון מספר N` (N = 1-based video index on the page); overrides to custom `title` when set. Circular play icon + "ללא קובץ" when no URL. Top chevron bar in edit only. Click opens URL in a new tab (edit/implement). Edit settings above placeholder: שם הסרטון, URL, פלטפורמה, הערות. Implement: notes under the block when set. **Preview (תצוגה)**: embeds via iframe when URL resolves (`src/lib/videoEmbed.ts` — YouTube id → embed URL; Panopto/`other` use URL as iframe src); falls back to placeholder when no embeddable URL; notes hidden. |
...
| `question` | questionType ('single_choice'), prompt, options[] ({id, text}), correctOptionId, notes | Edit: prompt + options + הערות. Implement: copy icon beside the prompt (visual left in RTL) copies the prompt; each option row has a copy icon that copies that option's text (both mark the component implemented); notes under the block when set. Preview: same read-only options, no copy controls / notes. |

Banner, video, and question share optional `props.notes` (הערות) — free-text for implementers. Editable in the block input/settings UI; shown under the component in implement when non-empty (label + red `text-danger` body, matching page notes); **hidden in preview**. Text blocks have no notes field. Stored in the existing `components.props` jsonb (no separate column).

TypeScript interfaces: `src/lib/types.ts` (`CourseViewMode = "edit" | "implement" | "review"`, `NotesProps` on every block props type, `PageType` / `derivePageType` for implement sidebar logos).
...
apps/course-builder/
  ...
  src/
    ...
    lib/
      ...
      api.ts                     → CRUD, reorder/renumber, rollups, getPageTypes, setComponentStatus / markImplemented, touchCourse (bump course updated_at on nested content)
