...
- Supabase (Postgres) via `@supabase/supabase-js` **directly from the browser** â€” no API routes needed (single-user MVP, no auth). Relational schema, no jsonb blob. RLS is enabled with open `anon`/`authenticated` policies (`006_enable_rls.sql`) so the SPA keeps working; data is still reachable with the anon key until login is added.
...
components
  ...
  type (text: 'banner' | 'video' | 'text' | 'question' | 'thumbnail')
  position (int)
...
| `question` | questionType ('single_choice'), prompt, options[] ({id, text}), correctOptionId, notes | Edit: prompt + options (no ×”×¢×¨×•×ª input). Implement: copy icon beside the prompt (visual left in RTL) copies the prompt; each option row has a copy icon that copies that option's text (both mark the component implemented); clicking either copy control briefly shows **הועתק** in place of the icon (~2s); legacy `notes` under the block when set. Preview: same read-only options, no copy controls / notes. |

| `thumbnail` | imageUrl, fileUrl, notes | Square image placeholder (aspect-square). Displays the image when `imageUrl` is set; clicking it opens `fileUrl` in a new tab (all modes). When no image URL or image fails to load: shows "ללא תמונה" placeholder. Edit: header popovers for image URL and file URL. Implement: legacy `notes` under block when set. Preview: same clickable image, notes hidden. |

Banner, video, question, and thumbnail may still carry optional `props.notes` in jsonb (legacy). **No edit-mode ×”×¢×¨×•×ª input** on blocks â€” use page-level "×”×¢×¨×•×ª ×œ×”×˜×ž×¢×”" and/or block comments instead. When non-empty, notes are still shown under the component in implement (label + red `text-danger` body); **hidden in preview**. Text blocks never had a notes field.
  ...
  src/
    ...
    components/
      ...
      blocks/
        ...
        BannerBlock.tsx, VideoBlock.tsx, TextBlock.tsx, QuestionBlock.tsx, ThumbnailBlock.tsx
...
The edit page header exposes a two-state swap control. The edit lesson overview exposes the same working/ready control for the whole lesson: the lesson is shown as ready only when all its pages are ready, and toggling it updates every page in that lesson together (the control is disabled for an empty lesson). In the edit sidebar, each page row shows a small low-opacity color square on the visual right (after the drag handle, before the page number/title): amber for `in_progress` and emerald for `ready_for_implementation`. The row background itself is unchanged by workflow status. Lesson rows use the same square indicator (after the drag handle, before the expand chevron): emerald when every page is ready, amber when at least one page is in progress (empty lessons count as in progress). New home/lesson/duplicated pages start `in_progress`. Migration [`supabase/migrations/005_page_workflow_status.sql`](../supabase/migrations/005_page_workflow_status.sql) backfills existing pages as `ready_for_implementation` so applying the migration does not unexpectedly remove existing implementer access.
...
- No auth. RLS is enabled, but policies still allow full CRUD for `anon` and `authenticated`, so the anon key has full read/write until login is added. Before sharing a public Vercel URL: enable Vercel deployment protection, or replace the open policies with authenticated-only access.
